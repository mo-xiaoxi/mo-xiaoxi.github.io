...
但是，我在测试的时候并没有发现这个问题。后来，与队友讨论和测试了一下，发现很神奇，不知道是为啥？MAC用户的问题？

我使用了57的chrome，macOS 10.12.3

![chrome_mac](http://momomoxiaoxi.com/img/post/0ctf/chrome_mac.png)

不会出现这种问题。

而队友的Ubuntu下56版本和57版本下，都会被拦截。

![chrome1](http://momomoxiaoxi.com/img/post/0ctf/chrome1.png)

![chrome2](http://momomoxiaoxi.com/img/post/0ctf/chrome2.png)

不知道为什么，求同是mac用户的，帮我测一下。是不是所有的macOS下Chrome下都不会出现这个问题？疑惑？
...
This vector set window.name with javascript:XSS payload and POST request on preview with `<svg onload=location=name`

After payload execution we got flag in http-access log: `flag{th1s_is_fr0m_a_re4l_cas3}`
